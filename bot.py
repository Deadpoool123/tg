...
TOKEN = "8183905855:AAG7WWVFnjLMHAUdIswAb9UVzHiK_OZrVas" 
...
# YENİ SABİT: Destek Hesap Linki
SUPPORT_LINK = "https://t.me/pinlysupport"
...
def main_menu(uid):
    ...
    # GÜNCELLENDİ: Dil ve Destek butonları yeni satıra eklendi
    kb.add(
        InlineKeyboardButton(t["language"], callback_data="language"),
        InlineKeyboardButton(t["support"], url=SUPPORT_LINK)
    )
    return kb
...
def callback(call):
    ...
    # GÜNCELLENDİ: İptal callback handler
        ...
        balance_float = get_sol_balance(public_key) # Fonksiyon adı düzeltildi
        balance_text = f"{balance_float:.8f} SOL" if balance_float >= 0 else t['api_fail_service'] 
